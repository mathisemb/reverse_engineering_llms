...
def dot_product_closest_token(model, embeddings):
    embedding_matrix = model.get_input_embeddings().weight.data
    print("embedding_matrix.shape:", embedding_matrix.shape) # [vocab_size, embedding_dim]
    print("embeddings.shape:", embeddings.shape) # [batch_size, num_tokens, embedding_dim]
    Ex = torch.matmul(embedding_matrix, embeddings)
    print("Ex.shape:", Ex.shape) # [vocab_size, num_tokens]
    nearest_tokens = torch.argmax(Ex, dim=-1) # Find the nearest tokens in the embedding space
    decoded_prompt = tokenizer.decode(nearest_tokens[0], skip_special_tokens=True)
    return nearest_tokens, decoded_prompt


def pseudo_inverse_closest_token(model, embeddings):
    embedding_matrix = model.get_input_embeddings().weight.data
    print("embedding_matrix.shape:", embedding_matrix.shape) # [vocab_size, embedding_dim]
    print("embeddings.shape:", embeddings.shape) # [batch_size, num_tokens, embedding_dim]
    pseudo_inverse = torch.linalg.pinv(embedding_matrix.T) # [vocab_size, embedding_dim]
    print("pseudo_inverse.shape:", pseudo_inverse.shape) # [vocab_size, embedding_dim]
    Epix = torch.matmul(pseudo_inverse, embeddings)
    print("Epix.shape:", Epix.shape) # [vocab_size, num_tokens]
    nearest_tokens = torch.argmax(Epix, dim=-1)
    decoded_prompt = tokenizer.decode(nearest_tokens[0], skip_special_tokens=True)
    return nearest_tokens, decoded_prompt
...
# INFERENCE WITH THE DOT PRODUCT CLOSEST TOKENS AND THE ORIGINAL MODEL
print("dot_product_closest_token:")
nearest_tokens, decoded_prompt = dot_product_closest_token(model, prompt_embeddings)
print("decoded_prompt:", decoded_prompt)
prompt_ids = tokenizer(decoded_prompt+input, return_tensors="pt").to(device)
model_outputs = init_model.generate(**prompt_ids, max_new_tokens=100)
print("model output with dot product closest tokens:", tokenizer.decode(model_outputs[0], skip_special_tokens=True), "\n")

# INFERENCE TEST WITH THE PSEUDO INVERSE CLOSEST TOKENS AND THE ORIGINAL MODEL
print("dot_product_closest_token:")
nearest_tokens, decoded_prompt = pseudo_inverse_closest_token(model, prompt_embeddings)
print("decoded_prompt:", decoded_prompt)
prompt_ids = tokenizer(decoded_prompt+input, return_tensors="pt").to(device)
model_outputs = init_model.generate(**prompt_ids, max_new_tokens=100)
print("model output with pseudo inverse closest tokens", tokenizer.decode(model_outputs[0], skip_special_tokens=True), "\n")
