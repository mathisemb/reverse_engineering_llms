...
res_filename = 'results/random_results' + date + '.txt'
...
with open(file_path, mode='r') as csv_file:
    ...
    for row in tqdm(islice(reader, number_of_examples), total=number_of_examples):
        ...
        # RANDOM TOKENS ADVERSARIAL PROMPT
        rand_adv_prompt = " ".join([tokenizer.decode(torch.randint(0, len(tokenizer), (1,)).squeeze().tolist()) for _ in range(num_virtual_tokens)])
   
        if with_target:
            text_output, jailbroken = check_for_attack_success_noref_with_target(init_model, tokenizer, rand_adv_prompt + input, target, test_prefixes, max_new_tokens)
        else:
            text_output, jailbroken = check_for_attack_success_noref(init_model, tokenizer, rand_adv_prompt + input, test_prefixes, max_new_tokens)
        ...
        with open(res_filename, 'a') as file: # write results in a file
            ...
            file.write("\nRandom adversarial prompt:\n" + rand_adv_prompt + "\n")
            file.write("\nRandom attack output:\n" + text_output + "\n")
